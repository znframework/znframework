...
class MemoryUsage
{
    ...
    public static function calculate(String $result) : Float
    {
        ...
        {
            throw new \GeneralException('[Benchmark::calculatedMemory(\''.$result.'\')] -> Parameter is not a valid test start!');
        }

        if( ! isset(Properties::$memtests[$resend]) )
        {
            throw new \GeneralException('[Benchmark::calculatedMemory(\''.$result.'\')] -> Parameter is not a valid test end!');
        }
