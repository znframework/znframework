...
class Xml
{
	...
	public static function create($version = "1.0", $charset = "iso-8859-8", $output = true)
	{
		...
		if( ! is_string($charset)) $charset = "iso-8859-8";
		if( ! is_bool($output)) $output = true;
		
		if(empty($version)) $version = "1.0";
		if(empty($charset)) $charset = "so-8859-8";
	...
	public static function add_element($add = '', $to = '')
	{
		...
		if(empty($to)) self::$dom->appendChild($add); else $to->appendChild($add); return $add;
	...
	public static function remove_element($add = '', $to = '')
	{
		...
		if( ! is_array($to))
		{
			if( ! is_object($to)) return false;
			if($add->hasChildNodes() <= $to->hasChildNodes()) return false;
			$add->removeChild($to); 
		}
		else
			foreach($to as $e)
	...
	{
		if( ! is_object($to))return false;
		if( ! (is_string($text) || is_numeric($text))) $text = "";
		
		$text = self::$dom->createTextNode($text);
		if(empty($to)) $to = self::$dom;
	...
	{
		if( ! is_object($element)) return false;
		if( ! (is_string($name) || is_numeric($name)))$name = "";
		if(empty($element))
			return false;
