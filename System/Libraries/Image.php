...
class Image
{
	...
	public static function thumb($fpath = '', $set = array())
	{
		...
		{
			self::$error = get_error('Image', 'image_file_not_found_error', $file_path);
		...
		{
			self::$error = get_error('Image', 'image_not_image_file_error', $file_path);
			report('Error', self::$error, 'ImageLibrary');
			return false;	
		}
	...
	public static function size($path = '', $width = 0, $height = 0)
	{
		...
		if(empty($path))
		{
			self::$error = get_error('Image', 'image_file_not_found_error', $path);
			report('Error', self::$error, 'ImageLibrary');
			return false;	
		}
